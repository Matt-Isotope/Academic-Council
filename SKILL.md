...
1. **Independence.** Write each figure as if it had not read the others. Never mention the other figures inside an individual block; comparisons belong only in the synthesis. If all six opinions end up saying the same thing in different voices without distinct reasoning grounded in their lenses, the exercise has failed.
...
- Give each subagent: the full text under review; only that figure's block from "The six figures" (its name, lens, must-do list, brake, verdict scale); Ground rules 1–9, including the language and register rule; its ID prefix; and the "Format of each figure's opinion" template.
- Tell each subagent to answer in the language used by the user, unless the user requested another language.
...
### Step 4: closing
One line of caution:
- In single-pass mode, the six opinions are generated by one model in one context, so their separation is instruction-based rather than architectural.
- In subagent mode, the figures are generated in separate contexts, but their conclusions and any items marked "[to verify]" should still be checked by the user.

Then offer, in one line, to save everything as a document and to run a second round on the revised version. Do not create files unless asked.
...
These counts are descriptive within each figure. They are not a common quantitative score and must not be compared mechanically across figures, which use different verdict scales.

**2. Convergences and divergences**
- *Convergences:* identify findings raised by two or more figures (with IDs). In subagent mode, when the figures were actually run in separate contexts, these may be treated as independently recurring concerns and given high priority. In single-pass mode, describe them as concordances rather than independent confirmations, because the same model generated all figures in one context. Do not automatically assign them higher reliability in single-pass mode.
- *Cross-reference check (single-pass mode):* check whether one figure's text refers to another figure's finding or wording (e.g. "same issue as the Skeptic flagged"). If so, do not present the resulting agreement as independent; report it as a concordance influenced by cross-reference instead.
