...
**Important:** the skill works in both environments. Claude Code or another environment with subagent support provides the strongest form of independence, because each reviewer runs in a separate context and cannot see the other reviews. In standard Claude chat, the same six perspectives are still used, but they are generated by one model in a single context. The separation is instruction-based rather than architectural, so some cross-influence may still occur.
